<?php
namespace PPhong\ProductDonate\Service;
...
/**
 * Class Configuration
 * @package PPhong\ProductDonate\Service
 */
class Configuration extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        return $this->_getConfig('amount', $store);
    }
    ...
    public function getBannerUrl($store = null)
    {
        ...
        if( !empty($configUrl) )
        {
            $bannerUrl = $this->storeManager->getStore()
                    ->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA ).'pp_productdonate/banner/'.$configUrl;
        }else {
